...
function variationToCanvasModel(v: any, index: number, originalPrompt: string): CanvasPostModel {
  const famMeta =
    (v.familyId && OFFICIAL_FAMILIES_META[v.familyId as keyof typeof OFFICIAL_FAMILIES_META]) ||
    OFFICIAL_FAMILIES_META["editorial-poster"];
  ...
  const imagePrompt =
    v.imagePrompt || `${originalPrompt}, professional photography, high resolution, minimalist editorial aesthetic`;
...
export default function StudioAppCleanPage() {
  ...
  const handleCreateSubmit = async (promptText: string, mode: "static" | "carousel") => {
    ...
    try {
      ...
      console.warn("[StudioAppClean] Fallback acionado:", err);
  ...
  const handleGenerateMore = async () => {
    ...
    try {
      ...
      console.warn("[StudioAppClean] Fallback inteligente de novas variações acionado:", err);
  ...
  return (
    <div className="min-h-screen w-full bg-[#06080D] text-white flex flex-col overflow-hidden font-sans">
      {stage === "create" && (
        <StudioCreateViewClean onSubmit={handleCreateSubmit} isLoading={isLoading} />
      )}
