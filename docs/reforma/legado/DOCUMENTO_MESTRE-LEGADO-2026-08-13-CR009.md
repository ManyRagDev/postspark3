...
> **CONFERÊNCIA GLOBAL — CUTOVER BLOQUEADO (2026-08-12).** A conferência independente das seis specs classificou o programa como **🟡 parcial — não aprovado para cutover**. Typecheck, 368 testes e build passam, mas os perfis de aceite visual `e2`/`e3`/`e5` reprovam; o orçamento de chamadas não cobre integralmente `creationMode: "execution"`; commit/refund financeiro podem falhar sem alterar a resposta terminal; o Supabase alvo mantém 6 requisitos críticos ausentes; e `verify:e2e` ainda não cobre a matriz completa nem export/readback/isolamento. Correções, ordem e provas de encerramento estão em [`docs/reforma/CONFERENCIA-GLOBAL-E-CORRECOES-2026-08-12.md`](./docs/reforma/CONFERENCIA-GLOBAL-E-CORRECOES-2026-08-12.md). **Não substituir este arquivo pelo novo mestre enquanto houver CR P0 aberta.**
...
> **Delta SPEC-006 (2026-08-12, aguardando conferência).** `npm run verify:e2e` cria evidência de ponta a ponta (runId, snapshot hashado, post persistido, relatório com chamadas/tokens/custo e p50/p95 do mesmo corpus); execuções reais registraram instabilidade transitória do provider e um bug real de persistência corrigido (NUL `\u0000` vindo do LLM é sanitizado no orquestrador). Corte documental preparado: cópia integral deste arquivo em `docs/reforma/legado/DOCUMENTO_MESTRE-LEGADO-2026-08-10.md` (SHA-256 `e137bff1…4593`, 227142 bytes) e rascunho do novo mestre em `docs/reforma/NOVO-DOCUMENTO-MESTRE.md`. O swap na raiz só ocorre após conferência do dono.
